...
type (*ville*) allie = {q : int ;r : int ;cid : int ; prod : unites ; tours_restants : int} ;;

let liste_ville_alliee = [] ;;

(* a utiliser avec listealliee = add_alliee *)
let update_ville_allie q r cid = 
    	if List.exists (cid_is cid) liste_ville_alliee then liste_ville_alliee
    	else {q =q ; r =r ; cid = cid ; prod = None ; tours_restants = -1} :: liste_ville_alliee ;;

(* a utiliser avec listealliee = rm_alliee *)
let rec rm_allie rmcid =
  let pred id alpha = alpha.cid != id in
    List.filter (pred rmcid) liste_ville_alliee
;;  

(* Liste villes ennemies *)
let liste_ville_ennemie = [] ;;

type (*ville*) ennemi = {q : int ;r : int ;cid : int } ;;

let add_ville_ennemi q r cid = 
  	let cid_is cid element = element.cid == cid in
    	if List.exists (cid_is cid) liste_ville_ennemie then liste_ville_ennemie
    	else  { q =q ; r =r ; cid = cid } :: liste_ville_ennemie ;;

let rec rm_ennemi rmcid =
  let pred id alpha = alpha.cid != id in
    List.filter (pred rmcid) liste_ville_ennemie
;;  
...
let traiter_set_visible args =
    ...
    | [ q ; r ; terrain ; "owned_city" ; cid ; jid ] -> if jid = our_jid then (fill_terrain "our_city" q r ; add_ville_allie q r cid) else (fill_terrain "their_city" q r ; add_ville_ennemi q r cid)
    | [ q ; r ; terrain ; "piece" ; jid ; pid ; ptid ; hits ] -> if jid = our_jid then (update_unite_alliee q r pid ptid hp) else (update_unite_ennemie q r pid ptid hp)
