...
let error2s = function
  ...
  | Lostid -> "Removed identifier (Lostid)"
...
struct
  module IdMap = Map.Make (struct type t = Elt.id let compare = Pervasives.compare end)
...
(*    Printf.printf "get_natsig: ids = %s\n%!" (List.fold_left (fun acu id -> acu ^ (id2s id) ^ ", ") "" (myget_ids elt)) ;
  ...
  let insert set elt =
    ...
    let new_index = myfold ids set.index
        begin fun index id ->
          assert (not (IdMap.mem id index)) ; (* The element is completely new. None of its ids can be in the set. *)
  ...
  let mymerge ?(force=false) (e1, intact) e2 =
    ...
    else raiserror Lostid (Printf.sprintf " merge %s and %s gives %s" (Elt.elt2s e1) (Elt.elt2s e2) (Elt.elt2s res)) ;
  ...
  let fold ?comparekey ?compare set acu f =
    ...
    let scompare = match (compare, comparekey) with
      | (None, None) -> (fun e1 e2 -> Pervasives.compare (myget_ids e1) (myget_ids e2))
      | (Some c, None) -> c
      | (None, Some c) -> (fun e1 e2 -> Pervasives.compare (c e1, myget_ids e1) (c e2, myget_ids e2)) (* c is not necessarily injective *)
