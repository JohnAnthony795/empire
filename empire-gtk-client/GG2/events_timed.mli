...
val sample: ?target: 'a prod -> ?only_change:bool -> period:float -> init:'a -> 'a prod -> 'a prod
...
val limit: ?target:'a prod -> ('a, 'c) rate_limit -> 'a prod -> 'a prod
(* Note that a rate-limited producer becomes asynchronous.
 * The output events it produces may happen later than its input events. *)

(* Another time-dependant producer: sends only 'stabilized' events.
 ...
 *
 * (As for 'limit', events are grouped by categories. See above)
 *)
val stabilized: ?target: 'a prod -> ('a, 'c) rate_limit -> 'a prod -> 'a prod
