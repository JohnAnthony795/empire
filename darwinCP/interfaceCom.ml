...
match action with
  | End_turn -> "end_turn"
  | Set_city_prod of (cid, ptid) -> "set_city_prod " ^ (soi cid) ^ " " ^ (unites_to_ptid ptid)
  | Move of (pid, did) -> "move " ^ (soi pid) ^ " " ^ (dir_to_string did)
...
let dir_to_string direction = match dir with 
	| Up -> "1"
	| Down -> "4"
	| Right -> "0"
	| Left -> "3"
